...
class AccessPanel_DBHttpRequest {
    ...
    function email_change_request_ins($respondent_id, $new_email){
        $data = array('onl_respondent_id' => $respondent_id, 'new_email' => $new_email);
        $res =  $this->request('email_change_request_ins', $data);
        return $res;
    }
    function email_change_request_confirm($request_id){
        $data = array('request_id' => $request_id);
        $res =  $this->request('email_change_request_confirm', $data);
        return $res;
    }
    ...
    function dic_locality_list($search_string, $lang = false){
        $data = array('search_string' => $search_string);
        if($lang) $data['lang'] = $lang;
        $res =  $this->request('dic_locality_list', $data);
        return $res['response'];
    }
    ...
    function onl_respondent_ins($last_name,$first_name,$sex,$birth_date,$cell_phone_number,$email,$city,
                                $parent_respondent_email=null,$ip=null, $cookie = null,
                                $dic_know_about_us_id = null, $dic_know_about_us_other = null, $useragent=null){
        $data = array('last_name' => iconv('cp1251', 'utf-8', $last_name),'first_name' => iconv('cp1251', 'utf-8', $first_name),
            'sex' => $sex,'birth_date' => $birth_date,
            'cell_phone_number' => $cell_phone_number,'email' => $email,'city' => $city,
    ...
    function complete_respondent_project($complete_code, $complete_type){
        $data = array('complete_code' => $complete_code, 'complete_type' => $complete_type, 'ip' => $_SERVER['REMOTE_ADDR'], 'useragent' => $_SERVER['HTTP_USER_AGENT']);
        $res =  $this->request('complete_respondent_project', $data);
        LogTo($res['response'], 'complete_respondent_project.txt');
        return $res['response'];
    }
    function ins_respondent_info_by_survey($request_id, $plugin_id = null, $cookie = null){
        $data = array('request_id' => $request_id, 'plugin_id' => $plugin_id, 'cookie' => $cookie, 'ip' => $_SERVER['REMOTE_ADDR'], 'useragent' => $_SERVER['HTTP_USER_AGENT']);
        $res =  $this->request('ins_respondent_info_by_survey', $data);
        LogTo($res, 'ins_respondent_info_by_survey.txt');
        return $res['response'];
    }
    ...
    function get_respondent_info($onl_respondent_id = null, $email = null, $urlid = null){
        $data = array();
