...
class AccessPanel_Respondent implements AccessPanel_Respondent_Interface{
	...
	public function getInfoByUrlid($urlid){

		$res = $this->dbRequest->get_respondent_info(null, null, $urlid);
		$res = array (
	...
	public function send_respondent_info_by_survey($request_id, $plugin_id = null, $cookie = null){
		$res = $this->dbRequest->ins_respondent_info_by_survey($request_id, $plugin_id, $cookie);
		return $res;
	}
	...
	public function email_change($respondent_id, $new_email){
		$new_email = strtolower($new_email);
		$res = $this->dbRequest ->email_change_request_ins($respondent_id, $new_email);
//		echo '<pre>';
//		var_dump($res); #~#
//		echo '</pre>';
		return $res;
	...
	public function email_success($request_id){
		$res = $this->dbRequest ->email_change_request_confirm($request_id);
		return $res;
	}
    ...
    public function is_phone_exists($phone){
        $res = $this->dbRequest ->is_phone_exists($phone);
        return $res;
    }
    public function dic_locality_list($search_string, $lang = false){
