...
switch ($action) {
	case 'access_package.authorize':
		...
		else
		{
			...
			header('Location: '.($to?$to:get_href('Authorization')).'?page_error_flag=1&backurl='.($_POST['backurl']?$_POST['backurl']:$to));
		}
		exit;
...
//*******
    case 'check_reg_email':
        $resp = ap_resp_init();
        echo json_encode(array('response' => $resp->Check_If_Email_Exists($_GET['email'])));
        exit;
        break;
//*******
	case 'check_reg_phone':
		$resp = ap_resp_init();
		echo json_encode(array('response' => $resp->is_phone_exists($_GET['phone'])));
		exit;
		break;
//*******

//*******
    case 'search_residence':
        $resp = ap_resp_init();
        $lang = $_GET['lang'] ? $_GET['lang'] : false;
        echo json_encode($resp->dic_locality_list($_GET['search_string'], $lang));
        exit;
        break;
//*******
    case 'getUrlid':
        echo json_encode(array('urlid'=> $_SESSION['respondent']['urlid']));
        exit;
        break;
    ...
    case 'check_old_pass':
		...
		$data = $resp->Authorize($_GET['email'], $_GET['pass']);
