...
function ap_select_dictionary_options($id_field_name, $value_field_name, $selected_value=null, $from, $filter_field_name=null, $filter=null, $html_el='option', $input_name='')
{
    ...
    if($html_el == 'option') {
        ...
        $ar[] = array('option_value'=>OPTION_VALUE_DEFAULT, 'option_text'=>"Pasirinkite iš išskleidžiamojo sąrašo", 'option_value_test'=>$selected_value);
    }
...
function ap_current_projects_list($rows_limit=null, $tpl='current_projects_row')
{
    ...
    foreach($ar as $i => $ar_row)
    {
        $ar[$i]['project_name'] = ($language=='LT'?conv_from_utf($ar[$i]['poll_subject_ukr']):conv_from_utf($ar[$i]['poll_subject']));
        $ar[$i]['project_description'] = ($language=='LT'?conv_from_utf($ar[$i]['poll_invite_ukr']):conv_from_utf($ar[$i]['poll_invite_rus']));
    }
...
function ap_project_points_label($points)
{
    ...
    return $points.' taškai';//.cons('point').$ar_sufix[$language][$sufix];
}
...
function ap_download_plugin_link($linkPage = false){
    if($linkPage == 'TNS_Top'){
        ...
        return 'href="'.TNS_TOP_DOWNLOAD.urlencode(str_replace("%urlid%",$_SESSION['respondent']['urlid'],TNS_TOP_DOWNLOAD_REFERRER)).'"';
    }
...
function ap_download_plugin_title()
{
    ...
    if(($br == 'Chrome' || $br == 'Opera15' || $br == 'Opera') && ($br != 'Android')) {
        ...
        $ret = $language=='RU' ? 'Загрузить exe-файл TNS Plugin' : 'Atsisiųskite exe-failą KANTAR TNS Plugin';
    }
    return $ret;
}
...
function ap_project_complete_result($code, $ctype){
    ...
    switch($ctype) {
        case -1:
            $res = nl2br_cms('project_already_pass_text');
            break;
        case -2:
            $res = nl2br_cms('project_was_close_text');
            break;
    }
