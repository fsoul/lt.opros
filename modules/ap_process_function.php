...
//        die('end');
        ...
        if($res['response_code'] == 301 || $res['response_code'] == 200){
            $error = 1;
        }elseif($res['response_code'] == 201){
            $error = 2;
        }elseif($res['response_code'] == '000'){
            $flag = 1;
        }
...
function ap_email_update_success(){
    $sid = $_GET['sid'];
    $resp = ap_resp_init();
    $res = $resp->email_success($sid);
    global $flag;
    ...
    LogTo($res, 'email_change_confirm.txt');
}
...
function ap_check_respondent_fields()
{
    ...
    if(!$_POST['is_mobile']) $error = ap_check_captcha('respondent_fields_captcha_code', $error);
    ...
    )
    {
        ...
        if($dbRequest -> is_phone_exists($_POST['cell_phone_number_'])){
            $error['cell_phone_number_'] = 'User with this phone is exist';
        }
    }
...
function check_all_fields($post){
    foreach($post as $key => $value){
        global $$key;
        $$key = trim($value);
    }
}
function clearTokenData($data){
...
function ap_process_recover_access(){
    global $error;

    $params = clearTokenData($_GET['token']);

    $resp = ap_resp_init();

    $resp_info = $resp->getInfoByUrlid($params['id']);

    if(md5($resp_info['email_']) != $params['emailHash']){
        $error['incorrect_link'] = 1;
        $error['page_error_flag'] = 1;
    }
    ...
    if(count($_POST)){
        ...
        if(!(bool)preg_match("/.+@.+\..+/", $email)){
            $error['email'] = 'Wrong email';
        }
        if(!(bool)preg_match("/^[A-Za-z0-9_]{6,}$/", $pass)){
            $error['pass'] = 'Wrong pass';
        }
        if($pass != $pass2){
            $error['pass_confirm'] = 'Wrong pass_confirm';
        }

        if(count($error) > 0){
            $error['page_error_flag'] = 1;
        }else{
            $pass_requset_id = $resp->Reset_Password_Get_Hash_Code($resp_info['email_']);
            if ($pass_requset_id) {
                $resp->Reset_Password($pass_requset_id, $pass);
            } else {
                $error['pass'] = 'Error in request password operation';
            }

            $resp->email_change($resp_info['respondent_id_'], $email);

            if (count($error) > 0) {
                $error['page_error_flag'] = 1;
            } else {
                header('Location: ' . get_href('access-restoring-successful'));
                exit;
            }
        }
    }
...
//            die();
            ...
            if (count($error) == 0){
                ...
                if ($res){
                    ...
                    if ($hash_code){
                        if ($resp->Reset_Password($hash_code, $_POST['passw'])){
                            $subj = page_cms('email_subject');
...
function verifyReCaptcha($response){
    $data = array(
        'secret' => "6Le2_CwUAAAAAOue5-XPXNPT34nw11VClbD7AUoF",
        'response' => $response
    );
...
function ap_process_new_password_update_form(){
    if (count($_POST) && array_key_exists('old_pass', $_POST)){
        $resp = ap_resp_init();
...
function ap_process_respondent_info_by_survey(){
    if (array_key_exists('plid', $_GET)){
        $plugin_id = $_GET['plid'];
        ...
        $resp = ap_resp_init();
        $res = $resp->send_respondent_info_by_survey($request_id, $plugin_id, $cookie = $cookie_str);
...
function ap_process_project_complete_by_url()
{
    ...
    if (array_key_exists('ID', $_GET))
    {
        ...
        if (array_key_exists('plid', $_GET) || array_key_exists('cookie', $_GET)){
            $plugin_id = $_GET['plid'];
            $cookie_str = $_GET['cookie'];
            $request_id = $_GET['ID'];
...
function setLSUrlid(){
    echo "<script>localStorage.setItem('urlid', {$_SESSION['respondent']['urlid']})";
}

function ap_process_profile_pool() {
    global $error, $language, $question_title, $question_code, $question_ar, $q_total, $cur_q, $percent;
    $percent = 0;
    $dbRequest = initHttpReq();
    ...
    $url_code = substr($_GET['ID'], 0, 10);
    $urlid = substr($_GET['ID'], 10, strlen($_GET['ID']));

    $user_info = $resp->getInfoByUrlid($urlid);
    if (is_null($user_info['respondent_id_'])) {
        $error = 1;
    }

    $resp_id = $user_info['respondent_id_'];
    ...
    if($last_question == 0 && $_POST['question_id'] != 'invite') {
        ...
        $question_title = nl2br_cms('suvey invitation '.$poll_id);
        ...
        $question_title = $language == 'RU'?$res['question_rus']:$res['question_ukr'];
        $question_code = $res['dic_ap_question_id'];
        $question_ar = $res['dic_answers'];
        $q_total = $res['poll_total_question_cnt'];
        $cur_q = $res['poll_complete_question_cnt']+1;
        $percent = $cur_q/$q_total*100;
    } else {
        $question_title = nl2br_cms('survey_complete '.$poll_id);
        $question_code = 0;
    }
}
